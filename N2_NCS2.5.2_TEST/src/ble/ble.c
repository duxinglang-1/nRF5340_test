...
#if 0	//xb add 2025-11-25 暂时屏蔽，防止编译报错
...
void APP_get_hr(uint8_t *buf, uint32_t len)
{
	...
	else
	{
		...
		case 0x02://单次测量心率
			...
			MCU_send_app_get_ppg_data(PPG_DATA_HR, &last_data.hr_rec.hr);
			break;
		}
	}
}
...
void APP_get_spo2(uint8_t *buf, uint32_t len)
{
	...
	else
	{
		...
		case 0x02://单次测量血氧
			get_cur_health_from_record(&last_data);
			MCU_send_app_get_ppg_data(PPG_DATA_SPO2, &last_data.spo2_rec.spo2);
			break;
		}
	}
}
...
void APP_get_bpt(uint8_t *buf, uint32_t len)
{
	...
	else
	{
		...
		case 0x02://单次测量血压
			get_cur_health_from_record(&last_data);
			data[0] = last_data.bpt_rec.bpt.systolic;
			data[1] = last_data.bpt_rec.bpt.diastolic;
...
void APP_get_temp(uint8_t *buf, uint32_t len)
{
	...
	else
	{
		...
		case 0x02://单次测量体温
			get_cur_health_from_record(&last_data);
			data[0] = last_data.temp_rec.deca_temp>>8;
			data[1] = (uint8_t)(last_data.temp_rec.deca_temp&0x00ff);
